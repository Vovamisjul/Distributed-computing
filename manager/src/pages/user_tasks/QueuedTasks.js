...
class QueuedTasks extends React.Component {
    ...
    async updateTasks() {
        try {
            let tasks = await (await controller.getUserQueuedTasks()).json();
            this.setState({
                tasks: tasks
            });
        } catch (e) {
    ...
    render() {
        return <ExpandingTaskList className="queuedTasks" title="Queued tasks" show={this.state.tasks.length > 0} countText={`You have ${this.state.tasks.length} queued tasks.`} absentText="There are no your tasks in queue">
            {
...
export default withRouter(QueuedTasks);